...
!!! note "Conceito: dois endereços distintos no MLflow"
    ...
    - **Tracking server**: onde os *runs* (experimentos) ficam, com params,
      métricas e artefatos brutos. Na Databricks, o tracking server da
    ...
    workshop vejam os runs no mesmo experiment, facilitando a comparação
    de execuções lado a lado na UI do MLflow.
...
!!! note "Conceito: `mlflow.sklearn.log_model` na API MLflow 3.x"
    ...
    - **`serialization_format="cloudpickle"`**: o formato padrão para
      ...
      o que é importante num ambiente de workshop onde versões do runtime podem
      variar ligeiramente entre participantes.
...
!!! tip "Curiosidade: por que `GradientBoostingRegressor` e não XGBoost?"
    ...
    (R² esperado ~0,75, dentro da banda [0,6 a 0,85]). O objetivo do workshop
...
!!! note "Conceito: o que é um validation gate?"
    ...
    **nenhuma versão de modelo é criada** e nenhum alias se move. O alias
    `@champion` permanece apontando para a última versão aprovada,
    ou simplesmente não existe se nenhuma versão foi aprovada ainda.
...
!!! note "Conceito: por que um gate determinístico de modelo único, sem champion-challenger?"
    ...
    São três peças fora do escopo deste workshop.
