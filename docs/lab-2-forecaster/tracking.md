...
**Dois endereços distintos no MLflow.** O MLflow tem duas áreas de armazenamento que podem ser configuradas de forma
independente:
...
**Por que Unity Catalog e não o registry antigo?** O Workspace Model Registry original não tem controle de acesso por modelo:
...
**Por que sem shuffle em séries temporais?** Como vimos na [geração do conjunto de dados](../lab-1-generate-data/index.md) (baseline ingênuo), em **previsão de séries temporais**
...
**`feats` inclui `price` como feature defasada.** `wl.DRIVERS` são os dez indicadores econômicos sintéticos (`demand_index`,
...
**O que é um MLflow *run*?** Um *run* é a unidade atômica de rastreamento no MLflow. Dentro do bloco
...
**`mlflow.sklearn.log_model` na API MLflow 3.x.** Na API do MLflow 3.x, o parâmetro correto é `name=`, **não**
...
**Por que `GradientBoostingRegressor` e não XGBoost?** GBR do scikit-learn é a escolha deliberada de **minimalismo didático**: está
...
**As três métricas de regressão logadas.** O notebook loga RMSE, MAE e R²:
