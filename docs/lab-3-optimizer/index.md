...
Para decidir **quanto comprar**, você precisa de um modelo de *otimização*: one that takes the predicted price as input and finds the quantity that minimizes total cost respecting hard constraints. Esses são problemas fundamentalmente diferentes — ML generaliza a partir de dados históricos, OR resolve uma formulação matemática explícita.
...
!!! warning "Atenção"
    Um modelo sem métricas de treino precisa de outro critério de qualidade antes do registro. Neste lab, usamos um **assert de viabilidade**: o solver deve retornar `status="optimal"` em um exemplo canônico. Se o ambiente estiver errado (solver ausente, dependências faltando, restrições mal especificadas), o notebook falha aqui — barulhento e antes do registro — em vez de registrar um modelo silenciosamente quebrado.
