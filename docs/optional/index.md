...
| `extra/cleanup.py` | Remove todos os recursos do workshop | — (mas leia o aviso antes de rodar) |
...
O notebook declara `torch` como dependência extra no bloco PEP 723 — é a única diferença de ambiente em relação ao Lab 2. O torch é pinado para download CPU-only para evitar o peso das wheels CUDA (vários gigabytes), que seriam desnecessários numa demo em serverless.
